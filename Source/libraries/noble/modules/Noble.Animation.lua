...
function Noble.Animation.new(__spritesheet, delay)
	...
	animation.startTime = playdate.getCurrentTimeMilliseconds()
	...
	function animation:addState(__name, __startFrame, __endFrame, __direction, __next, __loop, __onComplete)
	...
	--	end
	function animation:setState(__animationState, __reset, __unlessThisState)
		...
		local reset = true
		if (__reset ~= nil) then reset = __reset end

		if (reset == true) then
			self.current.loopFinished = false 
			self.current.completeCall = false
			self.startTime = playdate.getCurrentTimeMilliseconds() 
		end
		self.current = newState
		self.direction = newState.dir

	end

	function animation:setInverted(__flag)
		for i=1, self.imageTable:getLength() do
			local img = self.imageTable:getImage(i):setInverted(__flag)
		end
	end
	...
	function animation:draw(__x, __y, __advance)
		...
		elseif(self.currentFrame == self.current.endFrame + 1) then	-- End frame behavior.
			self.current.loopFinished = true
			...
			if (self.current.onComplete ~= nil and self.current.completeCall == false) then
				self.current.completeCall = true
		...
		if (__advance and not self.paused and self.currentFrame < self.current.endFrame + 1) then
			local elapsedTime = playdate.getCurrentTimeMilliseconds() - self.startTime
			local frame = self.current.startFrame + math.floor(elapsedTime / self.delay)
			if frame > self.current.endFrame + 1 then frame = self.current.endFrame + 1 end
