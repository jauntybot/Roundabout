...
--
function NobleSprite:init(__imageOrSpritesheet, __animated, __delay)
	...
	if (__imageOrSpritesheet ~= nil) then
		if (__animated == true) then
			...
			self.animation = Noble.Animation.new(__imageOrSpritesheet, __delay)
			-- for index, value in ipairs(__animStates) do
			-- 	self.animation:addState(value.name, value.startFrame, value.endFrame, value.next, value.loop, value.onComplete)
			-- end
		else
